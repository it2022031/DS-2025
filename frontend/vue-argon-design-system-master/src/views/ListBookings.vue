...
<!--    <div class="container">-->
<!--      <h2 class="text-center mb-4" style="color: #343a40;">📖 Bookings List</h2>-->
...
<!--          &lt;!&ndash; Review form: only if eligible and no review exists &ndash;&gt;-->
...
<!--          &lt;!&ndash; Show submitted review &ndash;&gt;-->
...
<template>
  <section class="list-rentals section bg-secondary py-5">
    <div class="container">
      ...
      <ul v-else class="rental-list">
        <li v-for="rental in filteredRentals" :key="rental.rentalId" class="rental-card">
          <div class="rental-info">
            <h3>Booking ID: {{ rental.rentalId }}</h3>
            <p><strong>Property's Name:</strong> {{ rental.propertyName }}</p>
            <p><strong>Owner's Name:</strong> {{ rental.ownerName }}</p>
...
export default {
  ...
  computed: {
    filteredRentals() {
      return this.rentals.filter(rental => {
        const matchesSearch =
            rental.propertyName && rental.propertyName.toLowerCase().includes(this.searchQuery.toLowerCase());
        const matchesStatus =
            this.selectedStatus === "ALL" || (rental.status && rental.status.toUpperCase() === this.selectedStatus);
  ...
  methods: {
    ...
    canLeaveReview(rental) {
      return rental.status && rental.status.toLowerCase() === 'approved' && new Date(rental.endDate) < new Date();
    },
    async fetchRentals() {
      this.loading = true;
      this.error = false;
      try {
        ...
        this.rentals.forEach(rental => {
          ...
          if (review) {
            this.$set(this.submittedReviews, rental.rentalId, review);
            this.$set(this.editReviews, rental.rentalId, { content: review.content, rating: review.rating });
          } else {
            this.$set(this.reviews, rental.rentalId, { content: '', rating: '' });
          }
    ...
    startEdit(rental) {
      const review = this.submittedReviews[rental.rentalId];
      this.editReviews[rental.rentalId] = { content: review.content, rating: review.rating };
      this.editing[rental.rentalId] = true;
    },
    cancelEdit(rental) {
      this.editing[rental.rentalId] = false;
